...
Gemini CLI handles images natively via `read_file` on image paths — pass image files directly to the model using `read_file` and Gemini's built-in vision will process them. No separate multimodal tool is required.
</Purpose>
...
<Inputs>
- `reference_images[]` (one or more image paths — pass each via `read_file` for Gemini vision)
- `generated_screenshot` (current output image path — pass via `read_file`)
...
- If `score < 90`, continue editing and rerun the visual-verdict skill before any further visual review pass.
- Do **not** treat the visual task as complete until the next screenshot clears the threshold.
</Threshold_And_Loop>

<Debug_Visualization>
When mismatch diagnosis is hard:
1. Keep the visual-verdict JSON as the authoritative decision.
2. Use pixel-level diff tooling (pixel diff / pixelmatch overlay) as a **secondary debug aid** to localize hotspots. Run via `run_shell_command` if a diff CLI is available.
