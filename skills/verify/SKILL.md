...
> **See also**: verification-before-completion (superpowers) for spec-conformance verification. This skill is focused on "run the app and observe behavior" — concrete runtime evidence, not spec compliance review.
...
3. If coverage is missing, run the narrowest direct verification commands available via `run_shell_command`.
