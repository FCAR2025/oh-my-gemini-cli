...
1. Select or infer event sequence (default):
   ...
   Coverage note: only `BeforeModel` and `AfterAgent` are registered to
   shipped scripts in `hooks/hooks.json`. Other events listed above are
   prompt-policy modeling targets — the dry-run reports what *would* fire
   if/when scripts are registered, and how lifecycle/lane policy would
   route them. Flag any phantom-event registration mismatch in `Skip
   Reasons` so operators do not assume enforcement that does not exist.
