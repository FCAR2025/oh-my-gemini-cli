---
name = "mcp-setup"
description = "Configure popular MCP servers for enhanced Gemini CLI agent capabilities"
---

# MCP Setup

Configure Model Context Protocol (MCP) servers to extend Gemini CLI's capabilities with external tools like web search, file system access, and GitHub integration.

## Overview

MCP servers provide additional tools that Gemini CLI agents can use. This skill helps you configure popular MCP servers using the `gemini mcp add` command-line interface, or by editing the `mcpServers` block in `gemini-extension.json` directly as an alternative.

## Step 1: Choose a Setup Path

Use a staged question flow with **one question at a time** and **no more than 3 options per question**.
...
**Options:**
1. **Recommended starter setup** - Fast path for the most common OmG MCP additions
...
If the user chooses **More server choices**, ask one more question:
...
Use the `gemini mcp add` command to configure each MCP server. The CLI automatically handles config updates and merging.

Alternatively, edit the `mcpServers` block in `gemini-extension.json` directly for project-scoped configuration.
...
NEXT STEPS:
1. Restart Gemini CLI for changes to take effect
...
- Run /omg:omg-doctor to diagnose issues

MANAGING MCP SERVERS:
- Add more servers: /omg:mcp-setup or `gemini mcp add ...`
...
If the custom server is meant to provide organization-specific reference material to OmG workflows, prefer a single tool named `get_company_context` that returns markdown via `{ context: string }`.

Example local registration:

```bash
gemini mcp add company-context -- node examples/vendor-mcp-server/server.mjs
```

Or add to the `mcpServers` block in `gemini-extension.json`:
...
```json
{
  "mcpServers": {
    "company-context": {
      "command": "node",
      "args": ["examples/vendor-mcp-server/server.mjs"]
    }
  }
}
```
