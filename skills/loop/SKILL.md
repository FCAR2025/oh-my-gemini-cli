...
5. Run `ultraqa` for the final cycle when risk is high, runtime/proxy-sensitive, or the user requests adversarial proof.
6. Never declare done when unresolved blocker, major item, open task, or missing verifier signoff remains.
7. Stop only when:
   - all criteria pass (done),
   - hard blocker exists (blocked),
   - max cycles reached (escalate).
8. Default max cycles: 3 for one invocation unless user overrides.
