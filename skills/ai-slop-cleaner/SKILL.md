...
4. **Run one smell-focused pass at a time**
   ...
   - Re-run targeted verification after each pass using `run_shell_command`.
...
- Use the `ai-slop-cleaner` skill with `<target>`
- Use the `ai-slop-cleaner` skill with `<target> --review`
- Use the `ai-slop-cleaner` skill with `<file-a> <file-b> <file-c>`
