...
1. Parse requested action:
   - `status`
   - `on` / `off`
   - `profile <minimal|balanced|strict>` (matches `OMG_HOOK_PROFILE` env vocabulary and `hooks/scripts/learn.js` enforcement; `fast` was an older alias and is no longer accepted)
   - `events`
