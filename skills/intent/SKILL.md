...
- runtime/model/tool fit unclear -> `/omg:capabilities` first (FCAR addition; gates `goal`/`team-assemble` for Antigravity proxy users — record the capability decision before automation entry)
- canonical automated flow when implementation is expected ->
  `/omg:team-assemble` (orchestrates plan -> prd -> taskboard -> exec -> verify -> fix; pre-flight with `/omg:capabilities` when runtime/model fit matters)
- single-shot autonomous delivery (assumed-approval, max-cycle bound) -> `/omg:goal` (FCAR: pre-flighted by `/omg:capabilities`)
- adversarial proof / release readiness / claim falsification -> `/omg:ultraqa` (FCAR: bounded verify -> diagnose -> fix loop)
