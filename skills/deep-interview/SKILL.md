...
<Purpose>
Deep Interview implements Ouroboros-inspired Socratic questioning with mathematical ambiguity scoring. It replaces vague ideas with crystal-clear specifications by asking targeted questions that expose hidden assumptions, measuring clarity across weighted dimensions, and refusing to proceed until ambiguity drops below the resolved threshold for this run. The output feeds into a gated pipeline: **deep-interview -> omg-plan consensus refinement -> pending approval -> explicitly approved execution**, ensuring maximum clarity before any mutation starts.
</Purpose>
...
- User wants a quick fix or single change -- delegate to omg-executor or ralph
...
- Gather codebase facts using Gemini CLI search tools (`read_many_files`, `glob`, `search_file_content`) BEFORE asking the user about them
...
- Do not proceed to execution until ambiguity is at or below the resolved threshold for this run and the user explicitly approves a scoped execution path
...
- Once ready, do **not** bridge into `omg-plan`, `autopilot`, `ralph`, or `team`. Instead write the mission/evaluator setup artifacts and invoke the `autoresearch` skill.
- After a successful handoff, announce the mission slug, evaluator command/script, max-runtime ceiling, and artifact location.
</Autoresearch_Mode>
...
2. **Detect brownfield vs greenfield**:
   - Use Gemini CLI search tools (`glob`, `read_many_files`) directly: check if cwd has existing source code, package files, or git history
   - If source files exist AND the user's idea references modifying/extending something: **brownfield**
   - Otherwise: **greenfield**
3. **For brownfield**: Build the first-round context before designing Round 1 questions:
   - Use `glob` and `search_file_content` to map relevant codebase areas, store as `codebase_context`.
...
3.5. **Load runtime settings**:
   - Read `.omg/config.jsonc` (project) for `deepInterview.ambiguityThreshold`; if undefined, use `0.2`
...
3.6. **Normalize oversized initial context before state init**:
   ...
   - Wait until the summary exists before ambiguity scoring, weakest-dimension selection, brownfield exploration prompts, or any bridge to `omg-plan`, `autopilot`, `ralph`, or `team`.
3.7. **Artifact path discipline**:
   - Final specs MUST be written to `.omg/specs/deep-interview-{slug}.md` exactly.
   - Ephemeral interview artifacts (scoring scratchpads, prompt-safe summaries, transient queues, resume metadata) belong in `.omg/state/`, never in the repo root or arbitrary working files.

4. **Initialize state** by writing `.omg/state/deep-interview-state.json` via `write_file`. Check `.omg/state/session-lock.json` first before writing:
...
Present the question clearly with the current ambiguity context:
...
**Scoring prompt** (use planning-lane model (gemini-3.1-pro-preview) at temperature 0.1 for consistency):
...
Greenfield: `ambiguity = 1 - (goal x 0.40 + constraints x 0.30 + criteria x 0.30)`
Brownfield: `ambiguity = 1 - (goal x 0.35 + constraints x 0.25 + criteria x 0.25 + context x 0.15)`
...
Update interview state with the new round, global scores, per-component `topology.components[].clarity_scores`, `topology.components[].weakest_dimension`, ontology snapshot, and `topology.last_targeted_component_id` by writing `.omg/state/deep-interview-state.json` via `write_file`.
...
1. **Generate the specification** using planning-lane model (gemini-3.1-pro-preview) with the prompt-safe transcript. If the full interview transcript or initial context is too large, include the summary plus all concrete decisions, acceptance criteria, unresolved gaps, and ontology snapshots; never overflow the prompt with raw oversized context.
2. **Write to file**: `.omg/specs/deep-interview-{slug}.md`
   ...
   - For ephemeral artifacts during interview rounds (for example scoring intermediate results, prompt-safe summaries, question queues, or resume metadata), use `.omg/state/`.
...
## Technical Context
{brownfield: relevant codebase findings from search tools}
...
**Autoresearch override:** if `--autoresearch` is active, skip the standard execution options below. The only valid bridge is invoking the `autoresearch` skill as described above.

After the spec is written, mark it `pending approval` and present execution options. Until the user selects an execution option, the deep-interview skill MUST NOT run mutation-oriented shell commands, edit source files, commit, push, open PRs, invoke execution skills, or delegate implementation tasks:

**Question:** "Your spec is ready (ambiguity: {score}%). How would you like to proceed?"

**Options:**

1. **Refine with omg-plan consensus (Recommended)**
   ...
   - Action: Only after the user selects this option, invoke the `plan` skill with `--consensus --direct` flags and the spec file path as context. The `--direct` flag skips the plan skill's interview phase (the deep interview already gathered requirements), while `--consensus` triggers the Planner/Architect/Critic loop. When consensus completes and produces a plan in `.omg/plans/`, stop with that plan marked `pending approval`; do not automatically invoke autopilot or any other execution skill.
   - Pipeline: `deep-interview spec -> explicit approval to refine -> plan --consensus --direct -> pending approval -> separate execution approval`
...
```
Stage 1: Deep Interview          Stage 2: omg-plan consensus        Stage 3: Separate approval
+---------------------+    +---------------------------+    +----------------------+
...
2. **omg-plan consensus** gates on *feasibility* -- is the approach architecturally sound?
...
```
[uses glob + search_file_content: "find authentication implementation"]
[receives: "Auth is in src/auth/ using JWT with passport.js"]

Question: "I found JWT authentication with passport.js in `src/auth/` (pattern match from file search).
For this new feature, should we extend the existing auth middleware or create
a separate authentication flow?"
```
Why good: Explored first using search tools, cited the repo evidence that triggered the question, then asked an informed confirmation question. Never asks the user what the code already reveals.
</Good>
...
```
Why bad: Should have used `glob` + `search_file_content` to find this. Never ask the user what the code already tells you.
</Bad>
...
If interrupted, use the `deep-interview` skill again. The skill reads state from `.omg/state/deep-interview-state.json` and resumes from the last completed round.
...
If the user chooses interview, autopilot invokes the `deep-interview` skill. When the interview completes and the user selects "Execute with autopilot", the spec becomes Phase 0 output and autopilot continues from Phase 1 (Planning).

## Approval-Gated Pipeline: deep-interview -> omg-plan -> pending approval

The recommended refinement path chains clarity and feasibility gates, then stops for explicit execution approval:

```
deep-interview "vague idea"
  -> Socratic Q&A until ambiguity <= threshold
  -> Spec written to .omg/specs/deep-interview-{slug}.md
  -> User explicitly selects "Refine with omg-plan consensus"
...
**The plan skill receives the spec with `--consensus --direct` flags** because the deep interview already did the requirements gathering. The `--direct` flag skips the interview phase and goes straight to Planner -> Architect -> Critic consensus. The consensus plan includes:
...
**Execution is a separate approval-gated step.** The deep-interview and plan skills must not auto-invoke autopilot, team, ralph, or any other execution skill merely because a spec or plan exists.
