...
- `install`: active Gemini CLI extension install/root diagnostics

Diagnostic protocol:
0. Tool schema guardrail: call `list_directory` with `dir_path`; call
   `read_file` and `write_file` with `file_path`; if `.omg/state/*` is ignored
   by native file tools, use a shell `cat`/`mkdir -p`/heredoc fallback instead of
...
3. Verify command readiness for core flows (`team`, `intent`, `workspace`, `taskboard`, `loop`, `rules`, `memory`, `hooks`, `notify`, `reasoning`, `approval`, `model`, `capabilities`, `ultraqa`).
...
5. Check state artifacts and drift risks under `.omg/state/` when available, including lane health, trust notes, capability notes, UltraQA evidence, and hook lifecycle policy.
6. Detect common failure patterns:
   - extension not loaded in current session
   - Gemini CLI runtime too old for current OmG assumptions (recommend `v0.42.0+` for current Plan Mode, extension, memory, model-routing, secure .env loading, and Auto Memory inbox behavior)
   - active terminal `HOME` differs from the intended Gemini runtime home, causing `gemini extensions list` to inspect the wrong install path
   - missing or stale root `GEMINI.md` import chain
   - mixed or stale OmG extension roots (for example: copied assets, duplicate local clones, stale installed extension contents, or linked roots diverging from the active repository)
   - command discovery mismatch, especially missing `capabilities` or `ultraqa`
   ...
   - proxy/runtime mismatch for separate Gemini API surfaces (Live, Files, Batch/cache, embeddings, media generation, TTS, Veo, Imagen, Lyria)
7. Produce concrete remediation actions in priority order.
8. Prefer extension-managed recovery paths first:
   - `gemini extensions list`
   - `gemini extensions validate <extension-root>`
   - reinstall, update, or `gemini extensions link <extension-root>` when the active root looks stale
   - keep one authoritative OmG hook registration path per event before editing shipped assets
9. If filesystem tools are available, write/update `.omg/state/doctor.md` using session-lock discipline.
   - Gemini CLI file tools require `file_path`; do not call `read_file` or `write_file` with `path`.
