...
[Docs](docs/guide/installation.md) | [Landing Page](https://joonghyun-lee-frieren.github.io/oh-my-antigravity/) | [History](docs/history.md)
...
> FCAR Edition: this public distribution is maintained by `FCAR2025` for Antigravity proxy + Gemini 3.1 workflow usage. Upstream project: https://github.com/Joonghyun-Lee-Frieren/oh-my-gemini-cli
...
gemini extensions link /path/to/oh-my-antigravity

HOME=/path/to/gemini-cli-home gemini extensions list
```

Use `gemini extensions validate /path/to/oh-my-antigravity` before linking local edits.

## What's New in v0.9.1-fcar.0
...
- **OMC-parity upgrade**: full hook surface + 16 new skills incl. `subagent-driven-development`.
  - **New SessionStart hook** (`hooks/scripts/session-start.js`) — fires on `startup|resume|clear` and auto-injects the new `using-oma` skill directive so every session knows how to find skills, agents, hooks, and `/oma:*` commands before the model speaks.
  - **New BeforeAgent hook** (`hooks/scripts/before-agent-keywords.js`) — parity with OMC's UserPromptSubmit keyword detector. Scans the operator prompt for OmA triggers (`autopilot`, `ralph`, `ulw`, `ccg`, `ralplan`, `deep-interview`, `tdd`, `brainstorm`, `subagent-driven-dev`, `mcp-setup`, `deepinit`, `visual-verdict`, `cancel`, etc.) and injects routing hints.
  - **New BeforeTool hook** (`hooks/scripts/before-tool-enforcer.js`) — denies fork-bombs / `mkfs` / `dd if=/dev/zero of=/dev/sd*` / `rm -rf /` and warns (without blocking) on `git push --force`, `git reset --hard`, `git clean -f`, `npm publish`, `cargo publish`, `kubectl delete`, `docker system prune`, `--no-verify`. Soft mode via `OMA_ENFORCER_SOFT=1`.
  ...
  - **New skill: `using-oma`** — the SessionStart directive itself. Lists the four capability surfaces (commands / skills / agents / hooks), the skill-priority order, the cancel latch, and OmA-specific discipline.
  - **New skill: `subagent-driven-development`** — ports superpowers' two-stage-review pattern to the OmA agent registry (`oma-executor` implementer → `oma-reviewer` spec compliance → `oma-verifier` code quality). Ships with `implementer-prompt.md`, `spec-reviewer-prompt.md`, and `code-quality-reviewer-prompt.md`.
  - **15 new OMC-parity skills**: `ultrawork`, `autopilot`, `autoresearch`, `ai-slop-cleaner`, `deep-interview`, `verify`, `trace`, `visual-verdict`, `ultragoal`, `deepinit`, `ccg`, `mcp-setup`, `wiki`, `learner`, `skillify`. Skill bodies adapted to Gemini-CLI tool names (`read_file`, `write_file`, `run_shell_command`, `glob`, `search_file_content`) and OmA agent registry (`oma-executor`, `oma-reviewer`, `oma-verifier`, `oma-debugger`, `oma-architect`, `oma-planner`, `oma-researcher`).
  - **Companion install**: the [superpowers](https://github.com/obra/superpowers) extension is now first-class alongside OmA. Recommended install: `gemini extensions install https://github.com/obra/superpowers` for the 14-skill core library (`brainstorming`, `test-driven-development`, `systematic-debugging`, `writing-plans`, `executing-plans`, `dispatching-parallel-agents`, `verification-before-completion`, `finishing-a-development-branch`, `requesting-code-review`, `receiving-code-review`, `using-git-worktrees`, `using-superpowers`, `subagent-driven-development`, `writing-skills`).
  - **Frontmatter**: all OMC ports converted from YAML to OmA's TOML-style frontmatter (`name = "value"` / `description = "value"`) per `scripts/check-skill-metadata.js`.
  - All three test gates (`test:version`, `test:skills`, `test:parity`) pass on the new surface.
- **Hook configuration details**: Disable any new hook via `OMA_DISABLED_HOOKS=session-start,keyword-detector,delegation-enforcer,evidence-capture` (comma list) or per-hook `OMA_SESSION_START_DISABLE=1` / `OMA_BEFORE_AGENT_DISABLE=1` / `OMA_BEFORE_TOOL_DISABLE=1` / `OMA_AFTER_TOOL_DISABLE=1`.
...
- Adopted upstream `/oma:goal` Ralph/Codex-style autonomous delivery loop while keeping FCAR `/oma:capabilities` and `/oma:ultraqa` ahead of automation entry.
- Adopted upstream automated/manual interview routing while preserving FCAR `capabilities` checkpoint between `intent` and `workspace`.
- Removed the upstream `oma-quota-watch-after-agent` hook (`hooks/scripts/after-agent-usage.js`) so OmA no longer emits noisy `usage=unavailable` lines; usage/quota visibility goes through Gemini CLI native `/model` or `/stats model`.
- Refreshed compatibility guidance to Gemini CLI `v0.42.0+` (preview/nightly tracked).

## What's New in v0.8.4-fcar.1

- Added `/oma:capabilities` to map tasks to Gemini-native strengths: large context, multimodal inputs, structured outputs, tool/function calling, MCP, grounding, URL context, Code Execution, Plan Mode, and extension hooks.
- Added `/oma:ultraqa` for adversarial verify -> diagnose -> fix loops with explicit evidence, edge-case checks, and single-writer state discipline.
- Tightened core anti-slop rules from the OmA/OMX pattern: small reversible diffs, evidence before claims, session-lock respect, and 1000-line soft / 1500-line hard file caps.
...
| Gemini capability planner | `/oma:capabilities` maps work to Gemini-native models, multimodal/tooling features, and proxy/runtime constraints |
| Adversarial QA loop | `/oma:ultraqa` cycles verify -> diagnose -> fix until the claim is proven, blocked, failed, or unknown |
...
| Model/tool choice is guessed instead of designed | `/oma:capabilities` selects Gemini-native features and calls out proxy-limited API surfaces before execution |
| Happy-path checks are mistaken for proof | `/oma:ultraqa` runs adversarial evidence loops and reports unknowns as unknowns |
...
```bash
export OMA_DISABLED_HOOKS=learn
```
...
| `/oma:capabilities` | Map a task to Gemini-native capabilities, model choices, and runtime/proxy constraints | Before work where model/tool/API fit matters |
...
| `/oma:ultraqa` | Run adversarial verify -> diagnose -> fix cycles with explicit evidence and bounded exits | Before final claims on risky, runtime/proxy, or release-critical work |
